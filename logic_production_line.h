...
public:
    ...
    {
        m_OnSucceed.Put(xOnSucceed);
        m_Onfailed.Put(xOnfailed);
        std::vector<mstr> xParams;
        m_Params.Put(xParams);
        this->OnInvoke();
    }
    ...
    SyncQueue<std::function<void(void)>> m_OnSucceed;  // 任务成功
    SyncQueue<std::function<void(void)>> m_Onfailed;   // 任务失败
    SyncQueue<std::vector<mstr>> m_Params;
} msLPL;
